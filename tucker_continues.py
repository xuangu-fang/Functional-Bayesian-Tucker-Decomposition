import numpy as np
import torch
from model_continues_mode_tensor import Continues_Mode_Tensor_Tucker
...
for fold_id in range(args.num_fold):
    ...
    if args.dataset == "US_temp":
        """Tucker version is not very stable for unnormalized data (US-Temp), so we normalize the data """
        us_data_max = data_dict["tr_y"].max()
        us_data_min = data_dict["tr_y"].min()

        us_data_mean = data_dict["tr_y"].mean()
        us_data_std = data_dict["tr_y"].std()

        data_dict["tr_y"] = (data_dict["tr_y"] - us_data_mean) / us_data_std
        data_dict["te_y"] = (data_dict["te_y"] - us_data_mean) / us_data_std

        # data_dict["tr_y"] = (data_dict["tr_y"] - us_data_min) / (us_data_max - us_data_min)
        # data_dict["te_y"] = (data_dict["te_y"] - us_data_min) / (us_data_max - us_data_min)

    model = Continues_Mode_Tensor_Tucker(hyper_dict, data_dict)
    ...
    if args.dataset == "US_temp":
        # test_result['rmse'] = test_result['rmse'] * (us_data_max - us_data_min)
        # test_result['MAE'] = test_result['MAE'] * (us_data_max - us_data_min)
        test_result['rmse'] = test_result['rmse'] * us_data_std
        test_result['MAE'] = test_result['MAE'] * us_data_std
