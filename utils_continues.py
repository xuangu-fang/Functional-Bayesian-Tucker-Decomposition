...
def parse_args_continues_tensor():

    description = "Bayesian continues-mode tensor factorization"
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument("--R_U",
                        type=int,
                        default=3,
                        help="dim of mode embeddings")
    parser.add_argument("--seed", type=int, default=321, help="rand_seed")
    parser.add_argument(
        "--num_fold",
        type=int,
        default=1,
    ...
    parser.add_argument(
        '--dataset',
        type=str,
        default='beijing_PM25',
        help='dataset name: beijing_PM25,beijing_PM10, beijing_NO2')
...
def make_log(args, hyper_dict, result_dict):
    ...
    f.write('\n Setting: R_U = %d, epoch = %d, kernel_type = %s,  lenthscale = %s, variance = %s,  \n'\
    %(hyper_dict['R_U'],hyper_dict['epoch'],hyper_dict['kernel'],hyper_dict['lengthscale_list'],hyper_dict['variance_list']))
...
def make_data_dict(hyper_dict, data_path, fold=0, args=None):
    ...
    data_dict["LDS_continues_mode_paras"] = make_LDS_continues_mode_paras(
        hyper_dict, data_dict)

    return data_dict


def make_LDS_continues_mode_paras(hyper_dict, data_dict):

    ndims = data_dict["ndims"]
    nmods = len(ndims)
    LDS_init_list = []
    never_seen_test_idx = data_dict["never_seen_test_idx"]

    for mode in range(nmods):
        ...
        LDS_init["fix_int"] = False

        train_id_CONTI_uni = list(
            np.sort(np.unique(data_dict["tr_ind_CONTI"][:, mode])))

        LDS_init["time_int_list"] = make_time_interval_list(train_id_CONTI_uni)

        LDS_init["id_CONTI_list"] = train_id_CONTI_uni

        CONTI_2_DISCT_dict = data_dict["CONTI_2_DISCT_dicts"][mode]

        LDS_init["id_DISCT_list"] = [
            CONTI_2_DISCT_dict[id_CONTI] for id_CONTI in train_id_CONTI_uni
        ]

        LDS_init["id_2_state_table"] = {
            key: id
            for id, key in enumerate(train_id_CONTI_uni)
        }

        LDS_init["never_seen_test_idx"] = never_seen_test_idx[mode]

        variance = hyper_dict["variance_list"][mode]
        lengthscale = hyper_dict["lengthscale_list"][mode]
        # variance = hyper_dict["variance_list"][0]
        # lengthscale = hyper_dict["lengthscale_list"][0]
        ...
        if hyper_dict["kernel"] == "Matern_21":
            ...
            LDS_init["P_0"] = LDS_init["P_inf"]
            LDS_init["m_0"] = torch.ones(2 * D, 1)

        else:
            raise Exception("mix-kernel is to be done in the furture")

        LDS_init_list.append(LDS_init)

    return LDS_init_list


def make_time_interval_list(train_time):

    time_int_list = [
        train_time[i + 1] - train_time[i] for i in range(len(train_time) - 1)
    ]
    return torch.tensor([0.0] + time_int_list)
